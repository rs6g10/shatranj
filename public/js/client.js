var Client = (function(window) {
    ...
    for (var i=0; i<gameState.validMoves.length; i++) {
      ...
      if (move.type === 'move') {
        if (move.pieceCode === piece && move.startSquare === square.attr('id')) {
            console.log(move.startSquare, square.attr('id'), move.endSquare)
            j++;
            square.removeClass('notAllowed');
            square.addClass('selected')
            $('#'+move.endSquare).addClass('valid-move');

        }
        else{
            console.log('here then?', j);
            squares.removeClass('notAllowed');
            if(j == 0){
                square.removeClass('selected');
                square.addClass('notAllowed')
            }
            else{
                square.addClass('selected');
            }
  ...
  var clearHighlights = function() {
    console.log('in clearHighlights');
      squares.removeClass('selected');
